...
# ㄱㅣ존은 주소를 받음
# 테스트할때 바꾸는 거는 주소
# 데이터를 받아오는 거는 걱정할 필요 없음

# 우리의 비즈니스 로직은 가져온 데이터를 파싱하는 것 
# 폼 가져오기 /// 원하는 형태로 가공하기  >>> 두개로 나누기

# 폼 가져오기
#  > 셀레늄 이용해서 데이터 호출 // return 파싱에 paser(data) >> html 파일로 저장

#  다른 함수 이거는 그 html 파일을 파싱 
#   data parser(data)




class TestGoogleForm(TestCase):
    @patch('requests.get')
